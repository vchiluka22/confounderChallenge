...
subtitle: "A visual case study in causal inference with observational vs randomized data"
format:
  html:
    theme: default
    css: report.css
...
doctor_style = {
    "Doc Dreamy": {"color": dreamy_color, "marker": "o", "label": "Doc Dreamy"},
    "Doc Duck": {"color": duck_color, "marker": "^", "label": "Doc Duck"},
}

fig, ax = plt.subplots(figsize=(7.5, 3.6))

for doctor_name, style in doctor_style.items():
    subset = patients_df.loc[patients_df["doctor_name"] == doctor_name, :]
    ax.scatter(
        subset["patient"],
...
mean_by_doctor = patients_df.groupby("doctor_name")["post_surgical_score"].mean()
ax.axhline(
    mean_by_doctor["Doc Dreamy"],
    color=dreamy_color,
    linestyle="--",
    linewidth=2,
    alpha=0.95,
...
ax.text(
    0.02,
    0.97,
    f"○ mean = {mean_by_doctor['Doc Dreamy']:.2f}",
...
ax.text(
    0.02,
    0.90,
    f"△ mean = {mean_by_doctor['Doc Duck']:.2f}",
    transform=ax.transAxes,
    ha="left",
    va="top",
    fontsize=9,
...
ax.set_title("Post-Surgical Symptom Score by Patient and Doctor", fontsize=11, pad=8)
ax.set_xlabel("Patient Number", fontsize=9)
ax.set_ylabel("Post-Surgical Symptom Score (Lower is Better)", fontsize=9)
...
ax.set_xlim(0, patients_df["patient"].max() + 5)
ax.margins(y=0.08)
...
for doctor_name, style in doctor_style.items():
    subset = patients_randomized_df.loc[
        patients_randomized_df["doctor_name"] == doctor_name, :
    ]
    ax.scatter(
        subset["patient"],
        subset["post_surgical_score"],
...
mean_by_doctor = patients_randomized_df.groupby("doctor_name")[
    "post_surgical_score"
].mean()
ax.axhline(
    mean_by_doctor["Doc Dreamy"],
...
ax.set_title("Post-Surgical Symptom Score (Randomized Assignment)", fontsize=11, pad=8)
ax.set_xlabel("Patient Number", fontsize=9)
ax.set_ylabel("Post-Surgical Symptom Score (Lower is Better)", fontsize=9)
ax.tick_params(axis="both", which="major", labelsize=9)
...
ax.set_xlim(0, patients_randomized_df["patient"].max() + 5)
ax.margins(y=0.08)
...
### Figure 3: Stratification by initial severity
...
ax.set_title("Post-Surgical Symptom Score by Patient Severity", fontsize=11, pad=8)
ax.set_xlabel("Initial Patient Severity", fontsize=9)
ax.set_ylabel("Post-Surgical Symptom Score (Lower is Better)", fontsize=9)
ax.tick_params(axis="both", which="major", labelsize=9)
...
## Key takeaway: defend against confounding
